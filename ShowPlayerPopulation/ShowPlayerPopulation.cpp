...
void ShowPlayerPopulation::RenderSettings() {
        ...
        ImGui::SameLine(0, ImGui::GetStyle().ItemInnerSpacing.x);
        open_popup2 |=
                ImGui::ColorButton("OverlayTextColor", settings.chosen_overlay_text_color, cef);
        if (open_popup2) {
        ...
        if (has_graph_data && (data_header_is_open = ImGui::CollapsingHeader("Data"))) {
                ...
                ImPlot::GetStyle().x_skip_gap_sz =
                        duration_cast<seconds>(minutes {minutes_gap}).count();
                if (ImPlot::BeginPlot(
...
static void ImGuiSettingsReadLine(
        ...
        if (sscanf(line,
                   "chosen_overlay_text_color={%f,%f,%f,%f}",
                   &colval1,
                   &colval2,
                   &colval3,
                   &colval4)
            == 4) {
                settings->chosen_overlay_text_color =
                        ImVec4 {colval1, colval2, colval3, colval4};
        }
}
