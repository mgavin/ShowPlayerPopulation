 ...
 *NO NO NO!
 *¯\(°_o)/¯乁( ◔ ౪◔)ㄏ [8/10]
 *  I can use csv reader.
 *
 *   Rotate files out, and maybe add a way to select which ones get data points.
 *   If a data file has too many data points, it takes too long to load the game.
 *
 *
 * OTHERS:
...
void ShowPlayerPopulation::init_cvars() {
      ...
      show_playlist_menu_cv.addOnValueChanged(
            [this](std::string old_value, CVarWrapper new_value) { show_in_playlist_menu = new_value.getBoolValue(); });
...
void ShowPlayerPopulation::init_hooked_events() {
      HookedEvents::AddHookedEvent(
            ...
            [this](std::string ev) {
                  ...
                  if (mw) {
                        ...
                        if (get_last_bank_entry().total_pop != tmpp && tmpp != 0 && !gameWrapper->IsInOnlineGame()) {
                              // THERE'S DATA!
                              record_population();
                              prepare_data();
      ...
      HookedEvents::AddHookedEvent("Function ProjectX.PsyNetConnection_X.EventDisconnected", [](std::string eventName) {
      ...
      if (mw) {
            ...
            for (const auto & element : bm_helper::playlist_ids_str) {
                  ...
                  if (entry.playlist_pop[element.first] == -1) {
                        ...
                        LOG("PLAYLIST MISSING#: {} . NAME_STRING: {}", static_cast<int>(element.first), element.second);
                  }
            }
...
void ShowPlayerPopulation::CHECK_NOW() {
      ...
      zoned_seconds now {current_zone(), time_point_cast<seconds>(system_clock::now())};
      if (auto time_waited = (now.get_local_time() - get_last_bank_entry().zt.get_local_time()); time_waited <= 5min) {
                        ...
                        with_Child(
                              ...
                              if (ImGui::GetWindowWidth() <= (ImGui::GetIO().DisplaySize.x / 2.0f)) {
                                    ...
                                    ImGui::BeginColumns(
                                          "populationnums_vert",
                                          2,
                                          ((settings.lock_overlay_borders) ? ImGuiColumnsFlags_NoResize : 0)
                                                | ((settings.show_overlay_borders) ? ImGuiColumnsFlags_NoBorder : 0));
                                    ...
                                    for (const std::pair<PlaylistId, int> & ppops : playlist_pops) {
                                          std::string playliststr = bm_helper::playlist_ids_str_spaced[ppops.first];
                                                      ...
                                                      ImGui::TextUnformatted(
                                                            std::vformat(
                                                                  ...
                                                                  std::make_format_args(
                                                                        bm_helper::playlist_ids_str_spaced[id]))
                                                                  .c_str());
                                                      ImGui::NextColumn();
